...
- **dsl 类**：不写文件，把 JSON 条目作为结构化结果返回，主控统一合并进 OP13.json。
- **engine 类**：不实现，返回 engineNeed 描述。
- **不要编译、不要碰 OP13.json、不要碰其他人的文件**。整合与编译由主控负责。

## 必读参考文件（动手前先 Read）
- 卡牌数据：`D:\Self\GrandUMI\_op13_cards.json`（含你负责卡的 number/effect 等）
...
- 现成手写脚本范例：`D:\Self\GrandUMI\服务端WebSocket\Effects\Scripted\` 目录下各 .cs（尤其 OP16_067_Otsuru.cs、OP15_029_Kuma.cs、OP13_108_Rosinante.cs）
- 触发枚举：`D:\Self\GrandUMI\服务端WebSocket\Effects\EffectTrigger.cs`

## 三种实现方式（按卡选最简者）
1. **dsl**：能用现有 DSL op 表达 → 产出该卡在 OP13.json 里的 JSON 条目（一个对象）。
...
public class OP13_XXX_名字 : IScriptedEffect
{
    public string CardNumber => "OP13-XXX";
...
- `scriptFileName`：approach=script 时你实际写入的文件名，形如 `OP13_007_Shanks.cs`（已写入 Scripted 目录）；否则空串
- `engineNeed`：approach=engine 时说明缺什么引擎能力；否则空串
- `summary`：一句话说明实现了什么 / 做了哪些简化
- `confidence`：high / medium / low

文件命名规则：`OP13_<三位号>_<罗马名>.cs`，类名与文件名一致（如 `OP13_007_Shanks`）。罗马名简短即可，避免与已存在文件重名（先 Glob 看 Scripted 目录）。
