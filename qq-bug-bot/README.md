...
- 这是独立于下方“入群后新人验证”的配置；同一群可以同时启用两者。普通主动申请在 OneBot 明确审批成功后，会先持久保存“申请人、目标群、已审核邀请人”的不可改写授权；新人入群后仍会进入登记验证，并且只能重复填写申请阶段已审核的邀请人 QQ。审批失败或响应中断时，预备记录不会授权群消息回答，也不会触发提示或踢人。
...
- 只有目标群的 OneBot `group_increase` / `group_decrease` 通知，或同群已被 OneBot 明确同意的持久加群申请，才能建立登记资格；重复入群通知不会重置当前倒计时，真实离群后再次入群会创建新一轮验证。若审批成功后的入群通知丢失，同一申请人发出的真实群消息可在授权有效期内恢复会话；未确认或已过期的审批记录不能恢复。
