...
可显式开启固定网咖群激活码领取。仅主助理 `s-蛇（3215228879）` 在群 `1039789967`、`1104489180` 接受顶层纯文本“领码”：每一条不同的可信 OneBot 消息原子占用一个未发激活码并 @领取人发到原群；同一 QQ 发送新的消息可以再次领取。引用、转发、附件、CQ 嵌入、机器人消息和缺少可信 `message_id` 的事件都不会触发。
...
| `activation_code_claim_enabled` | 是否启用固定网咖群领码；默认关闭，只有库存完成受控导入并核验后才能在私密配置中改为 `true` |
...
- 目标群号和执行账号写死为 `1039789967`、`1104489180` 与 `s-蛇（3215228879）`，不从群消息或可扩张的群白名单读取。副助理和其他账号即使收到相同事件也不会占用库存。
- 命令先做 NFKC 规范化和首尾空白清理，再精确比较“领码”。只允许顶层结构化 `text` 段；任何 `at`、`reply`、`forward`、图片、附件、字符串消息或 CQ 内容都会失败关闭。预占库存前还会通过不走缓存的 `get_group_member_info` 核对群号、QQ 和 `is_robot=false`；查询失败、响应矛盾或机器人账号都不占用库存。
- 库存单独保存在 `/data/activation_codes.db`，生产文件权限为 `600`，受控备份目录权限为 `700`。真实码不得进入 Git、配置、部署包、日志、错误信息或测试快照。
- 每条可信消息在调用 `send_group_msg` 前以 `BEGIN IMMEDIATE` 原子占用一个码并持久化消息幂等键。OneBot 明确成功后记录返回消息号；明确拒绝、超时、断线、取消、进程崩溃或成功后落库失败都不会把该码放回库存，也不会重发同一事件。结果未知时宁可隔离一个码，不能把可能已经发出的码再分配。
- 库存导入只从标准输入接收，先完整验证 UTF-8、17 位字母数字横线格式、批内唯一性、预期数量和 SHA-256，再分别生成导入前与导入后 SQLite 在线备份。重复导入同一文件不会新增库存，也不会改写已领取状态。示例中的占位值必须由部署人员从离线校验结果填写：

```sh
docker compose exec -T bug-bot python import_activation_codes.py \
  --stdin --expected-count COUNT --expected-sha256 SHA256 \
  --backup-dir /data/activation-code-backups
```

完成导入后只核对脚本输出的数量与 SHA-256；不得打印或查询激活码明文。随后才可在私密 `config.server.json` 中设置 `"activation_code_claim_enabled": true` 并重建 `bug-bot`。真实群验收不应主动发送“领码”，避免无故消耗库存。

导入前快照只适合在首次启用前回退。功能启用后不得把旧的激活码数据库快照直接恢复为权威库，否则快照之后已发出的码可能被再次分配；若权威库损坏或丢失，应先关闭领取功能，把历史导入批次全部视为已消耗，再以一批全新的码建立新库。任何恢复都不得仅凭旧快照重新开放库存。
