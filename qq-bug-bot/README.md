...
可按群开启新人邀请人验证。新人入群后，机器人会主动 @新人，要求其真正 @“释迦的助理”并回答邀请人 QQ。机器人只通过 OneBot 当前群成员列表核验邀请人；候选 QQ 不在群时继续追问，核验成功后持久记录。首次提示确认送达一段时间后若仍未完成，机器人只追问一次；没有回答期限，也绝不会自动移出成员，玩家之后仍可随时补充。机器人断线或重启不会丢失待验证会话。
...
- 每个 `add` 事件都会先调用 `get_group_member_list`，并强制传递 `no_cache=true`。NapCat 对普通主动申请和“群成员邀请好友且需管理员审核”均上报 `add`，但前者的 `user_id` 是申请人，后者是当前群内邀请人。
- 事件 `user_id` 已在实时成员列表中时，机器人会使用事件原始 `flag`、`sub_type=add` 直接同意，不解析 `comment`；这也覆盖动作响应不确定后申请人已经入群的重复事件。`user_id` 不在群时才把事件当作普通主动申请，接受纯 QQ 号，以及 `QQ号：123456789`、`邀请人QQ是 123456789` 和 NapCat 的“问题/答案”包装；答案必须只包含一位完整 QQ，不能填写申请人自己或机器人，并复用同一份实时成员列表核验所填邀请人。
- 成员查询失败、事件缺少有效 `flag`、`user_id` 或机器人身份，或审批动作失败时只记录日志并保持待审批，不会把未知结果当作成功。同一进程内，只有 OneBot 已确认成功的事件才按群和 `flag` 去重；过期 `flag` 等动作失败不会写入成功记录，允许 NapCat 重投后重试。
- 这是独立于下方“入群后新人验证”的配置；同一群可以同时启用两者。普通主动申请在 OneBot 明确审批成功后，会先持久保存“申请人、目标群、已审核邀请人”的不可改写授权；新人入群后仍会进入登记验证，并且只能重复填写申请阶段已审核的邀请人 QQ。审批失败或响应中断时，未确认的预备记录不会授权“入群通知丢失后由群消息恢复会话”，也不会据此触发提示或追问。
...
- 只有目标群的 OneBot `group_increase` / `group_decrease` 通知，或同群已被 OneBot 明确同意的持久加群申请，才能建立登记资格；重复入群通知不会重置当前一次追问的时间，真实离群后再次入群会创建新一轮验证。若审批成功后的入群通知丢失，同一申请人发出的真实群消息可在授权有效期内恢复会话；未确认或已过期的审批记录不能恢复。
...
- 目标验证群中，真实 @机器人并严格发送 `邀请人QQ：123456789`、但当前没有可用验证会话时，机器人会明确提示联系管理员核对；审批结果仍待确认时会提示稍后重试。两种提示都只消费登记消息，不创建会话、不写入回答，也不会把登记请求误当普通聊天。普通含 QQ 聊天、复制的文本 `@`、附加其他正文或多个 QQ 均不会命中该提示。
- 若该新人由自动审批进入，登记 QQ 必须与申请阶段已经实时核验的邀请人完全一致；不同 QQ 只会收到纠正提示，不会覆盖审核记录，也不会写入回答表。
- 成员列表接口只用于核验玩家已经提交的邀请人答案；接口失败时既不会通过，也不会丢失答案，SQLite 会保留答案并自动重试。一次追问不会查询成员列表，更不存在任何自动移出成员的调用。
