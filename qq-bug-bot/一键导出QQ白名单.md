...
需要手工导入最新白名单时，直接双击仓库根目录的 `一键导出QQ白名单.cmd`。入口固定读取 `UMI网咖（297542853）` 与 `UMI网咖2店（524996856）`，不能通过命令行改成其他群；成功后会显示完整文件路径、两群原始人数、排除机器人后人数、跨群重复人数、最终人数、实时拉取时间和 SHA-256，并在资源管理器中选中新生成的 `qq-whitelist-297542853-524996856-YYYYMMDD-HHmmss-fff-live.json`。窗口会保留，便于查看结果。
...
只有前置群信息、成员列表、后置群信息的群号、群名和人数完全一致，且两个固定群名分别精确为 `UMI网咖` 与 `UMI网咖2店`、全部成员均为目标群内唯一的 5–12 位纯数字 QQ 时才接受。拉取期间人数变化会有限重试；错误群、群名漂移、空名单、重复 QQ、无效 QQ、串群记录、SSH/容器/NapCat/OneBot 不可用均直接失败，绝不回退到旧导出。
