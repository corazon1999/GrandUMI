...
[[ -f /etc/letsencrypt/live/candidate.grand-umi.com/fullchain.pem ]] || { echo "缺少 candidate.grand-umi.com 证书" >&2; exit 1; }
...
rm -f /etc/nginx/sites-enabled/grandumi-candidate /etc/nginx/sites-enabled/default
...
curl -kfsS --resolve candidate.grand-umi.com:443:127.0.0.1 https://candidate.grand-umi.com/backend/ready >/dev/null
echo "新正式服双域名入口已预置：IP=$production_ip"
