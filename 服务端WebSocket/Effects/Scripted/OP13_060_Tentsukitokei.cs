...
/// <summary>
/// OP13-060 天月时（角色 / 暗 / 和之国・罗杰海盗团，cost2 power3000）
/// 我方拥有的特征中包含〈罗杰海盗团〉的角色因对方的效果将要被KO的场合，
///   可以改为将此角色（自身）放置到废弃区，使该角色不会被KO。
///
/// 实现说明：
///   - OnAllyWillBeKOd 守护者钩子；"因对方的效果"判 KOReason=="effect" 且 KOActingSide 为对方。
///   - 被守护目标须含〈罗杰海盗团〉特征。置换成本：自身放置废弃区，MarkPreventKO 取消KO。
/// </summary>
public class OP13_060_Tentsukitokei : IScriptedEffect
{
    public string CardNumber => "OP13-060";
    ...
    public async Task Resolve(EffectContext ctx)
    {
        ...
        if (!victim.Info.HasKeyword("罗杰海盗团")) return;

        bool use = await ctx.Prompts.ConfirmOptional(ctx.OwnerIndex,
            "天月时：将此角色放置到废弃区，使该〈罗杰海盗团〉角色不会被KO？");
        if (!use) return;
