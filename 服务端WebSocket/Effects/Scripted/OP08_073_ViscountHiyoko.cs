...
/// <summary>
/// OP08-073 小鸡子爵（角色 / 暗 5 费 6000，大妈海盗团）
/// 【对方的回合中】【KO时】咚!!-1（可以将我方场上指定数量的咚‼放回咚‼卡组）：
///   将我方卡组中最多 1 张费用不高于 6 的"公鸡伯爵"登场。之后，切洗卡组。
///
/// 实现说明：与 OP08-071 同构。
///   - 触发：OnKO；【对方的回合中】通过 CurrentTurnPlayer != owner 限定。
///   - 成本：咚!!-1（ReturnDonToDeck）。需我方场上有咚可放回。
///   - 收益：从卡组登场最多 1 张费用≤6 的"公鸡伯爵"，之后切洗卡组。
/// </summary>
public class OP08_073_ViscountHiyoko : IScriptedEffect
{
    public string CardNumber => "OP08-073";
    ...
    public async Task Resolve(EffectContext ctx)
    {
        ...
        // 候选：卡组中费用≤6 的"公鸡伯爵"
        var cands = me.Deck
            .Where(c => c.Info.Kind == CardKind.Character && c.Info.Cost <= 6 && c.MatchesName("公鸡伯爵"))
            .ToList();
        if (cands.Count == 0) return;

        bool use = await ctx.Prompts.ConfirmOptional(ctx.OwnerIndex,
            "小鸡子爵【KO时】：咚!!-1，从卡组登场 1 张费用≤6 的\"公鸡伯爵\"？");
        if (!use) return;
        ...
        // 之后切洗卡组
        ctx.Engine?.ShuffleDeck(me, ctx.OwnerIndex, "OP08-073_ko");
    }
}
