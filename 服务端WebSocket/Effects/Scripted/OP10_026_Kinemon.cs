...
/// <summary>
/// OP10-026 锦卫门（角色，力量 1000）
/// 【启动主要】可以将此角色和我方废弃区中 1 张力量为 0 的"锦卫门"自选顺序放回卡组最下方：
///   将我方手牌中最多 1 张费用为 6 的"锦卫门"登场。
///
/// 实现说明：
///   - 成本：将此角色 + 废弃区 1 张力量 0 的"锦卫门"放回卡组最下方。
///     需废弃区存在符合条件的"锦卫门"，否则无法支付成本，不发动。
///   - "自选顺序放回卡组最下方"实现为依次放底（对实战影响极小）。
///   - 收益：从手牌登场最多 1 张费用 6 的"锦卫门"。
/// </summary>
public class OP10_026_Kinemon : IScriptedEffect
{
    public string CardNumber => "OP10-026";
    ...
    public async Task Resolve(EffectContext ctx)
    {
        ...
        // 成本前置：废弃区需有力量 0 的"锦卫门"
        var trashKinemon = me.Trash
            .Where(c => c.Info.Power == 0 && c.MatchesName("锦卫门"))
            .ToList();
        if (trashKinemon.Count == 0) return;

        bool use = await ctx.Prompts.ConfirmOptional(ctx.OwnerIndex,
            "锦卫门【启动主要】：将此角色和废弃区 1 张力量 0 的\"锦卫门\"放回卡组底，登场手牌中 1 张费用 6 的\"锦卫门\"？");
        if (!use) return;

        // 选择废弃区中作为成本的 1 张力量 0 的"锦卫门"
