...
/// <summary>
/// OP01-044 夏齐（角色 / 风 / 心脏海盗团）
/// 【阻挡者】（纯关键词，由引擎处理）
/// 【登场时】我方场上不存在"佩金"的场合，将我方手牌中最多 1 张"佩金"登场。
///
/// 实现说明：
///   - 仅实现【登场时】主动效果（阻挡者为关键词，引擎处理）。
///   - 条件：我方场上（角色区）不存在名为"佩金"的角色。
///   - 从手牌选最多 1 张名为"佩金"的角色用 PlayFromHandFree 登场。
/// </summary>
public class OP01_044_Shachi : IScriptedEffect
{
    public string CardNumber => "OP01-044";
    ...
    public async Task Resolve(EffectContext ctx)
    {
        ...
        // 我方场上已存在"佩金"则不发动
        if (me.Characters.Any(c => c.MatchesName("佩金"))) return;

        var candidates = me.Hand.Where(c =>
            c.Info.Kind == CardKind.Character && c.MatchesName("佩金")).ToList();
