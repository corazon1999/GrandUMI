...
/// <summary>
/// OP14-028 强尼（角色，风）
/// 【我方的回合中】当此角色转为休息状态时，将对方最多 1 张处于休息状态且费用不高于 2 的角色 KO。
///
/// 实现说明：
///   - 用 OnCharRested watcher（仅当被横置的是本卡自身、且为我方回合时触发）。
///   - 候选 = 对方处于休息状态(IsTapped) 且原本费用≤2 的角色；KO 用 AtomicOps.KO。
/// </summary>
public class OP14_028_Johnny : IScriptedEffect
{
    public string CardNumber => "OP14-028";
    ...
    public async Task Resolve(EffectContext ctx)
    {
        ...
        var chosen = await ctx.Prompts.ChooseCards(owner, "OpponentCharacter",
            "将对方最多 1 张休息状态且费用≤2 的角色 KO",
            cands.Select(c => c.Id.ToString()).ToList(), 0, 1);
        if (chosen.Count > 0)
        {
            var tgt = cands.First(c => c.Id.ToString() == chosen[0]);
            AtomicOps.KO(ctx.State, 1 - owner, tgt);
        }
    }
}
