  ...
  return (
    <div id="rank-faction-rules" className="mt-2 rounded-xl border border-violet-800/60 bg-black/20 p-3 text-xs leading-5 text-gray-300">
      ...
      <ul className="mt-3 list-disc space-y-1 pl-4 text-gray-400">
        ...
        <li>悬赏金达到 10亿但未达到 20亿贝里时，基础胜负增加或减少 2500万贝里；连胜奖励最高 1250万贝里，连败保护和分差修正上限均为 630万贝里。</li>
        <li>悬赏金达到 20亿贝里后，基础胜负增加或减少 5000万贝里；连胜奖励最高 2500万贝里，连败保护和分差修正上限均为 1260万贝里。</li>
        <li>终结至少 3 连胜的玩家时，会按对方赛前悬赏金档位额外获得一次连胜赏金：1亿5000万、3亿、6亿、10亿和20亿档分别为 200万、400万、750万、1250万和2500万贝里。</li>
        <li>悬赏金达到 1亿5000万、3亿、6亿、10亿或 20亿贝里后，对应档位会成为永久保底线，之后不会再掉回上一档。</li>
                ...
                {matchState === "idle" && (
                  <>
                    ...
                    <p className={`text-xs leading-5 ${isHex ? "text-cyan-200/80" : "text-gray-500"}`}>
                      {isHex
                        ? "海克斯使用狂野公开卡池与禁卡表；每局随机生成 3 段可重复的银/金/棱彩品质序列，双方共享，并分别在自己的第 1、3、6 回合开始前私密三选一。每轮三个候选各可刷新 1 次，已向该玩家展示过的海克斯整局不会再次出现。"
                        : rankedMode === "standard"
