...
test("休闲匹配可选择标准与狂野且新客户端默认标准", async () => {
  ...
  assert.match(lobby, /狂野休闲可使用角标 1 等已轮换卡牌，但仍执行官网禁卡表；禁卡仅好友或房间对战可用/);
});
...
test("标准与狂野休闲贯穿牌组校验、独立队列和房间快照", async () => {
  ...
  assert.match(bridge, /MatchKind\.Ranked => DeckValidator\.FormatStandardRanked/);
  assert.match(bridge, /MatchKind\.CasualStandard => DeckValidator\.FormatStandard/);
  assert.match(bridge, /_ => DeckValidator\.FormatPublicUnrestricted/);
  assert.match(bridge, /MatchKind\.Friendly or MatchKind\.RoomCode => DeckValidator\.FormatUnrestricted/);
...
test("旧 casual 协议继续进入狂野休闲但不能绕过官网禁卡", async () => {
  ...
  assert.match(bridge, /历史 casual 协议值继续表示狂野休闲匹配，但公开对战仍执行官网禁卡表/);
  assert.match(bridge, /_ => "casual"/);
  assert.match(bridge, /_ => MatchQueue/);
  assert.match(bridge, /DeckValidator\.FormatPublicUnrestricted/);
});
