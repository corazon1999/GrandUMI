...
test("是否发动效果使用旋转画布容器尺寸而非物理视口", async () => {
  ...
  assert.match(source, /100cqh - 5\.5rem/);
  assert.match(source, /var\(--layout-safe-left, 0px\)/);
  assert.match(source, /var\(--layout-safe-right, 0px\)/);
  assert.match(source, /const minimizedEffectPromptStyle = \{[\s\S]*?bottom: "calc\(4\.5rem \+ var\(--layout-safe-bottom, 0px\)\)"/);
});
...
test("效果确认框使用紧凑横向布局与至少 44px 的操作按钮", async () => {
  ...
  assert.match(source, /const promptDecisionHeightClass = rotateQuarterTurn \? "h-16" : "h-12"/);
  assert.match(source, /onClick=\{\(\) => submitServerPrompt\(\["1"\]\)\}[\s\S]*?\$\{promptDecisionHeightClass\} min-w-20[\s\S]*?aria-label="取消"/);
  assert.match(source, /onClick=\{\(\) => submitServerPrompt\(\["0"\], true\)\}[\s\S]*?\$\{promptDecisionHeightClass\} min-w-20[\s\S]*?aria-label="确认"/);
  assert.match(source, /\$\{promptToggleSizeClass\} shrink-0[\s\S]*?aria-label="收起效果确认框"/);
...
test("选择面板隐藏与恢复按钮适配手机竖屏安全区", async () => {
  ...
  assert.match(source, /const promptToggleSizeClass = rotateQuarterTurn \? "h-16 w-16" : "h-12 w-12"/);
  assert.equal(source.match(/\$\{promptToggleSizeClass\} items-center justify-center rounded-full bg-slate-800\/90/g)?.length, 2);
});
