...
`changelog-cache/pending/` 中留下记录，再提交当前任务涉及的文件并运行统一部署入口：

```powershell
.\deploy-test.ps1
```

统一入口会直接部署并验证 `https://test.grand-umi.com`。日常开发、验证和交付不再考虑候选环境，
也不调用候选服部署脚本。日常自动部署不要求提前清空 `pending/`，不得把无关工作区改动混入提交；
只有测试服外网 HTTP 验证通过，才算完成交付部署。测试服部署不等于正式发布批准。
...
`deploy-hk.ps1` 只保留紧急发布用途，必须显式添加 `-Emergency`。它会跳过在线房间排空等待，
但仍要求目标是已经完整验证并成功部署到测试服的最新 `main`，且更新日志已归档、当前正式版
是目标祖先、共享账号权威健康。远端通过目标提交中的版本化脚本预构建并切换 A/B 槽位，
不会 checkout 或覆盖 `/opt/grandumi` 中承载历史产物的工作树。服务器配置源文件位于 `ops/server/`。

## 文档索引

- [架构文档](文档/架构文档.md) —— 分层 / 模块 / 卡牌系统 / 网络协议
- [启动流程](文档/启动流程.md) —— 详细的启动命令与端口配置
- [正式服发布与低延迟保障手册](文档/正式服发布与低延迟保障手册.md) —— 正常发布、紧急发布、延迟验收与事故处置
