...
统一入口会先部署并验证新香港服务器候选环境 `https://candidate.grand-umi.com`，成功后才继续部署
`https://test.grand-umi.com`。日常自动部署不要求提前清空 `pending/`，也不得把无关工作区改动混入提交；
只有两个环境的外网 HTTP 验证均通过，才算完成交付部署。候选环境和测试服部署都不等于正式发布批准。
